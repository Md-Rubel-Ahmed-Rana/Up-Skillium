...
const CreateCourseOutline = () => {
  ...
  const handleEditRow = (record: Partial<DataType> & { key: React.Key }) => {
    form.setFieldsValue({ title: "", serial: 0, ...record });
  ...
  const columns = [
    ...
    { title: "Module Name", dataIndex: "title", key: "title", editable: true },
    {
      title: "Actions",
      key: "action",
    ...
    <div className="mt-4 space-y-2">
      <div className="flex flex-col">
        ...
        <div className="flex items-center gap-2 mt-2 text-lg font-semibold">
          ...
          <h4>{courseTitle || "Not selected"}</h4>
        </div>
        <NewModuleForm modules={modules} setModules={setModules} />
      </div>
      <div className="bg-white p-5 rounded-md shadow-md">
        <div className="flex items-center gap-2 mb-4">
          <h2 className="text-xl font-medium">Module List</h2>
        ...
        <Form form={form} component={false}>
          <Table<DataType>
            ...
            pagination={{ onChange: handleCancelEditRow, pageSize: 10 }}
          />
        </Form>
      </div>
    </div>
